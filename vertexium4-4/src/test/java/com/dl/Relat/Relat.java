...
public class Relat implements Serializable {
    ...
    @Test
    public void rr2() throws AccumuloSecurityException, AccumuloException {
        ...
        Query query = defaultGraph.query(rootAuth).hasId(strings.toArray(new String[0]));
    ...
    public void rr1() throws AccumuloSecurityException, AccumuloException {
        AccumuloGraph defaultGraph = VertexiumConfig.defaultGraph;
        AccumuloAuthorizations rootAuth = AuthUtils.getRootAuth(defaultGraph);
        Vertex vertex = defaultGraph.getVertex("123", rootAuth);
        Iterable<Property> name = vertex.getProperties("name");
        name.forEach(x->{
            System.out.println(x);
        });
//        VertexBuilder vertex = defaultGraph.prepareVertex("123", Visibility.EMPTY);
//        vertex.setProperty("name","张三",Visibility.EMPTY);
//        vertex.setProperty("name","李四",Visibility.EMPTY);
//        vertex.save(rootAuth);
        defaultGraph.flush();
        System.exit(1);
//
...
//        });
    ...
    public void tmp3() throws IOException, ClassNotFoundException {
        ...
        Date crtea = (Date) next.getProperty("crtea").getValue();
