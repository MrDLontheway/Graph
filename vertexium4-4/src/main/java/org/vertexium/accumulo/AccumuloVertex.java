...
public class AccumuloVertex extends AccumuloElement implements Vertex {
    ...
    public AccumuloVertex(
            org.vertexium.accumulo.AccumuloGraph graph,
            String vertexId,
    ...
    public static Vertex createFromIteratorValue(
        ...
        try {
            ...
            org.vertexium.accumulo.util.DataInputStreamUtils.decodeHeader(in, ElementData.TYPE_ID_VERTEX);
            vertexId = org.vertexium.accumulo.util.DataInputStreamUtils.decodeString(in);
            timestamp = in.readLong();
            vertexVisibility = new Visibility(org.vertexium.accumulo.util.DataInputStreamUtils.decodeString(in));

            ImmutableSet<String> hiddenVisibilityStrings = org.vertexium.accumulo.util.DataInputStreamUtils.decodeStringSet(in);
            ...
            List<MetadataEntry> metadataEntries = org.vertexium.accumulo.util.DataInputStreamUtils.decodeMetadataEntries(in);
            properties = org.vertexium.accumulo.util.DataInputStreamUtils.decodeProperties(graph, in, metadataEntries, fetchHints);

            ImmutableSet<String> extendedDataTableNames = org.vertexium.accumulo.util.DataInputStreamUtils.decodeStringSet(in);
            outEdges = org.vertexium.accumulo.util.DataInputStreamUtils.decodeEdges(in, graph.getNameSubstitutionStrategy());
    ...
    private Iterable<EdgeInfo> accumuloEdgeInfosToEdgeInfos(Iterable<Map.Entry<Text, org.vertexium.accumulo.iterator.model.EdgeInfo>> edgeInfos, Direction direction) {
        return new ConvertingIterable<Map.Entry<Text, org.vertexium.accumulo.iterator.model.EdgeInfo>, EdgeInfo>(edgeInfos) {
            @Override
