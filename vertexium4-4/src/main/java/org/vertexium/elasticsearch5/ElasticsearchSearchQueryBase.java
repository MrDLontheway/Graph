 ...
 * @Version 1.0
 */
public class ElasticsearchSearchQueryBase extends org.vertexium.query.QueryBase {
...
//    public List<QueryBuilder> sholdQueryBuilders = new ArrayList<>();
    ...
    private SearchRequestBuilder buildQuery(EnumSet<ElasticsearchDocumentType> elementType, FetchHints fetchHints, boolean includeAggregations) {
        ...
        System.out.println("searching for: " + toString());
        ...
        //TODO DL CHANGE
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery().must(query).filter(filterBuilder);
        if(qbs.size()>0){
            qbs.forEach(x->{
                boolQueryBuilder.must(x);
            });
        }
//        BoolQueryBuilder boolQuerytmp = QueryBuilders.boolQuery();
//        if(sholdQueryBuilders.size()>0){
//            qbs.forEach(x->{
//                boolQuerytmp.should(x);
//            });
//        }
//        boolQueryBuilder.must(boolQuerytmp);
        //DL CHANGGE
        SearchRequestBuilder searchRequestBuilder = getClient()
                ...
                //.setQuery(QueryBuilders.boolQuery().must(query).filter(filterBuilder)) //old
                .storedFields(
                        org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME,
                        org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_TYPE_FIELD_NAME,
                        org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXTENDED_DATA_TABLE_NAME_FIELD_NAME,
                        org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXTENDED_DATA_TABLE_ROW_ID_FIELD_NAME
                );

        if (fetchHints.equals(FetchHints.NONE)) {
            searchRequestBuilder.storedFields(
                    org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.OUT_VERTEX_ID_FIELD_NAME,
                    org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.IN_VERTEX_ID_FIELD_NAME,
                    org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME
            );
        }
    ...
    protected List<QueryBuilder> getFilters(EnumSet<ElasticsearchDocumentType> elementTypes, FetchHints fetchHints) {
        ...
        if ((elementTypes == null || elementTypes.contains(ElasticsearchDocumentType.EDGE))
            ...
            filters.add(QueryBuilders.termsQuery(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME, edgeLabelsArray));
        }
        ...
        if (getParameters() instanceof QueryStringQueryParameters) {
            org.vertexium.elasticsearch5.Elasticsearch5SearchIndex es = (org.vertexium.elasticsearch5.Elasticsearch5SearchIndex) ((GraphWithSearchIndex) getGraph()).getSearchIndex();
    ...
    protected void applySort(SearchRequestBuilder q) {
        ...
        for (SortContainer sortContainer : getParameters().getSortContainers()) {
            ...
            if (Element.ID_PROPERTY_NAME.equals(sortContainer.propertyName)) {
                q.addSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME, esOrder);
                sortedById = true;
            } else if (Edge.LABEL_PROPERTY_NAME.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else if (Edge.OUT_VERTEX_ID_PROPERTY_NAME.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.OUT_VERTEX_ID_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else if (Edge.IN_VERTEX_ID_PROPERTY_NAME.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.IN_VERTEX_ID_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
                ...
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXTENDED_DATA_TABLE_NAME_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else if (ExtendedDataRow.ROW_ID.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXTENDED_DATA_TABLE_ROW_ID_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else if (ExtendedDataRow.ELEMENT_ID.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else if (ExtendedDataRow.ELEMENT_TYPE.equals(sortContainer.propertyName)) {
                q.addSort(
                        SortBuilders.fieldSort(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_TYPE_FIELD_NAME)
                                .unmappedType(KEYWORD_UNMAPPED_TYPE)
                                .order(esOrder)
                );
            } else {
                org.vertexium.PropertyDefinition propertyDefinition = getGraph().getPropertyDefinition(sortContainer.propertyName);
                ...
                if (propertyNames.length > 1) {
                    ...
                    List<String> fieldNames = Arrays.stream(propertyNames).map(propertyName ->
                            propertyName + (propertyDefinition.getDataType() == String.class ? org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXACT_MATCH_PROPERTY_NAME_SUFFIX : "")
                    ...
                    if (propertyDefinition.getDataType() == String.class) {
                        sortField += org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXACT_MATCH_PROPERTY_NAME_SUFFIX;
                        unmappedType = KEYWORD_UNMAPPED_TYPE;
                    }
                    q.addSort(
...
//        }
    ...
    @Override
    public QueryResultsIterable<? extends org.vertexium.VertexiumObject> search(EnumSet<VertexiumObjectType> objectTypes, FetchHints fetchHints) {
        validateQueryString();
    ...
    private QueryResultsIterable<? extends org.vertexium.VertexiumObject> searchScroll(EnumSet<VertexiumObjectType> objectTypes, FetchHints fetchHints) {
        return new QueryInfiniteScrollIterable<org.vertexium.VertexiumObject>(objectTypes, fetchHints) {
            @Override
            protected ElasticsearchGraphQueryIterable<org.vertexium.VertexiumObject> searchResponseToIterable(SearchResponse searchResponse) {
    ...
    private QueryResultsIterable<? extends org.vertexium.VertexiumObject> searchPaged(EnumSet<VertexiumObjectType> objectTypes, FetchHints fetchHints) {
        return new PagingIterable<org.vertexium.VertexiumObject>(getParameters().getSkip(), getParameters().getLimit(), pageSize) {
            @Override
            protected ElasticsearchGraphQueryIterable<org.vertexium.VertexiumObject> getPageIterable(int skip, int limit, boolean includeAggregations) {
    ...
    private ElasticsearchGraphQueryIterable<org.vertexium.VertexiumObject> searchResponseToVertexiumObjectIterable(SearchResponse response, FetchHints fetchHints) {
        ...
        org.vertexium.query.QueryParameters filterParameters = getParameters().clone();
        filterParameters.setSkip(0); // ES already did a skip
        List<Iterable<? extends org.vertexium.VertexiumObject>> items = new ArrayList<>();
        ...
        if (ids.getVertexIds().size() > 0) {
            if (fetchHints.equals(FetchHints.NONE)) {
                ...
                Iterable<? extends org.vertexium.VertexiumObject> vertices = getGraph().getVertices(ids.getVertexIds(), fetchHints, authorizations);
                items.add(vertices);
            }
        }
        if (ids.getEdgeIds().size() > 0) {
            if (fetchHints.equals(FetchHints.NONE)) {
                ...
                Iterable<? extends org.vertexium.VertexiumObject> edges = getGraph().getEdges(ids.getEdgeIds(), fetchHints, authorizations);
                items.add(edges);
            }
        }
        if (ids.getExtendedDataIds().size() > 0) {
            Iterable<? extends org.vertexium.VertexiumObject> extendedDataRows = getGraph().getExtendedData(ids.getExtendedDataIds(), authorizations);
            items.add(extendedDataRows);
        }
        Iterable<org.vertexium.VertexiumObject> vertexiumObjects = new JoinIterable<>(items);
        List<org.vertexium.VertexiumObject> sortedVertexiumObjects = sortVertexiumObjectsByResultOrder(vertexiumObjects, ids.getIds());
    ...
    private List<ElasticsearchVertex> getElasticsearchVertices(SearchHits hits, FetchHints fetchHints, Authorizations authorizations) {
        return stream(hits)
                .map(hit -> {
                    String elementId = hit.getField(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME).getValue();
    ...
    private List<ElasticsearchEdge> getElasticsearchEdges(SearchHits hits, FetchHints fetchHints, Authorizations authorizations) {
        return stream(hits)
                .map(hit -> {
                    String inVertexId = hit.getField(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.IN_VERTEX_ID_FIELD_NAME).getValue();
                    String outVertexId = hit.getField(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.OUT_VERTEX_ID_FIELD_NAME).getValue();
                    String label = hit.getField(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME).getValue();
                    String elementId = hit.getField(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME).getValue();
    ...
    private <T extends org.vertexium.VertexiumObject> List<T> sortVertexiumObjectsByResultOrder(Iterable<T> vertexiumObjects, List<String> ids) {
    ...
    protected <T> ElasticsearchGraphQueryIterable<T> createIterable(
            SearchResponse response,
            org.vertexium.query.QueryParameters filterParameters,
    ...
    protected QueryBuilder getFilterForHasNotPropertyContainer(HasNotPropertyContainer hasNotProperty) {
        org.vertexium.PropertyDefinition[] propertyDefinitions = StreamSupport.stream(hasNotProperty.getKeys().spliterator(), false)
                .map(this::getPropertyDefinition)
                .filter(Objects::nonNull)
                .toArray(org.vertexium.PropertyDefinition[]::new);
        ...
        List<QueryBuilder> filters = new ArrayList<>();
        for (org.vertexium.PropertyDefinition propDef : propertyDefinitions) {
            ...
            for (String propertyName : propertyNames) {
                ...
                if (GeoShape.class.isAssignableFrom(propDef.getDataType())) {
                    filters.add(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.GEO_PROPERTY_NAME_SUFFIX)));
                } else if (isExactMatchPropertyDefinition(propDef)) {
                    filters.add(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXACT_MATCH_PROPERTY_NAME_SUFFIX)));
                }
            }
        }
    ...
    private QueryBuilder getFilterForHasExtendedDataFilter(HasExtendedDataFilter has) {
        ...
        if (has.getElementId() != null) {
            boolQuery.must(QueryBuilders.termQuery(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME, has.getElementId()));
            hasQuery = true;
        }
        if (has.getTableName() != null) {
            boolQuery.must(QueryBuilders.termQuery(org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXTENDED_DATA_TABLE_NAME_FIELD_NAME, has.getTableName()));
            hasQuery = true;
        }
        if (!hasQuery) {
    ...
    protected QueryBuilder getFilterForHasAuthorizationContainer(HasAuthorizationContainer hasAuthorization) {
        ...
        List<QueryBuilder> filters = new ArrayList<>();
        for (org.vertexium.PropertyDefinition propertyDefinition : graph.getPropertyDefinitions()) {
        ...
        List<String> internalFields = Arrays.asList(
                org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_TYPE_FIELD_NAME,
                org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.HIDDEN_VERTEX_FIELD_NAME,
                org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.HIDDEN_PROPERTY_FIELD_NAME
        );
    ...
    protected QueryBuilder getFilterForHasPropertyContainer(HasPropertyContainer hasProperty) {
        org.vertexium.PropertyDefinition[] propertyDefinitions = StreamSupport.stream(hasProperty.getKeys().spliterator(), false)
                .map(this::getPropertyDefinition)
                .filter(Objects::nonNull)
                .toArray(org.vertexium.PropertyDefinition[]::new);
        ...
        for (org.vertexium.PropertyDefinition propDef : propertyDefinitions) {
            ...
            for (String propertyName : propertyNames) {
                ...
                if (GeoShape.class.isAssignableFrom(propDef.getDataType())) {
                    filters.add(QueryBuilders.existsQuery(propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.GEO_PROPERTY_NAME_SUFFIX));
                } else if (isExactMatchPropertyDefinition(propDef)) {
                    filters.add(QueryBuilders.existsQuery(propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXACT_MATCH_PROPERTY_NAME_SUFFIX));
                }
            }
        }
    ...
    protected QueryBuilder getFilterForGeoComparePredicate(GeoCompare compare, HasValueContainer has) {
        org.vertexium.PropertyDefinition[] propertyDefinitions = StreamSupport.stream(has.getKeys().spliterator(), false)
                .map(this::getPropertyDefinition)
                .filter(Objects::nonNull)
                .toArray(org.vertexium.PropertyDefinition[]::new);
        ...
        List<QueryBuilder> filters = new ArrayList<>();
        for (org.vertexium.PropertyDefinition propertyDefinition : propertyDefinitions) {
            ...
            String[] propertyNames = Arrays.stream(getPropertyNames(propertyDefinition.getPropertyName()))
                    .map(propertyName -> propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.GEO_PROPERTY_NAME_SUFFIX)
                    .toArray(String[]::new);
            for (String propertyName : propertyNames) {
    ...
    private QueryBuilder getFilterForProperty(Contains contains, HasValueContainer has, String propertyName, Object value) {
        if (Element.ID_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME;
        } else if (Edge.LABEL_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME;
        } else if (Edge.OUT_VERTEX_ID_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.OUT_VERTEX_ID_FIELD_NAME;
        } else if (Edge.IN_VERTEX_ID_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.IN_VERTEX_ID_FIELD_NAME;
            ...
            propertyName = propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EXACT_MATCH_PROPERTY_NAME_SUFFIX;
        }
        switch (contains) {
            case IN://TODO contains IN
    ...
    private QueryBuilder getFilterForProperty(Compare compare, HasValueContainer has, String propertyName, Object value) {
        //todo dl
        org.vertexium.PropertyDefinition definition = this.getGraph().getPropertyDefinition(propertyName.substring(0,propertyName.lastIndexOf("_")));
        if (Element.ID_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.ELEMENT_ID_FIELD_NAME;
        } else if (Edge.LABEL_PROPERTY_NAME.equals(propertyName)) {
            propertyName = org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.EDGE_LABEL_FIELD_NAME;
    ...
    public org.vertexium.elasticsearch5.Elasticsearch5SearchIndex getSearchIndex() {
        return (org.vertexium.elasticsearch5.Elasticsearch5SearchIndex) ((GraphWithSearchIndex) getGraph()).getSearchIndex();
    }
    ...
    protected QueryBuilder getFilterBuilder(List<QueryBuilder> filters) {
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
        for (QueryBuilder filter : filters) {
            boolQuery.must(filter);//TODO DLTMP
        }
        return boolQuery;
    }
    ...
    protected List<AggregationBuilder> getElasticsearchGeohashAggregations(GeohashAggregation agg) {
        List<AggregationBuilder> aggs = new ArrayList<>();
        org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(agg.getFieldName());
        if (propertyDefinition == null) {
        ...
        for (String propertyName : getPropertyNames(agg.getFieldName())) {
            ...
            geoHashAgg.field(propertyName + org.vertexium.elasticsearch5.Elasticsearch5SearchIndex.GEO_POINT_PROPERTY_NAME_SUFFIX);
    ...
    protected List<AggregationBuilder> getElasticsearchTermsAggregations(TermsAggregation agg) {
        ...
        if (Element.ID_PROPERTY_NAME.equals(fieldName)
            ...
            org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(fieldName);
    ...
    private boolean isExactMatchPropertyDefinition(org.vertexium.PropertyDefinition propertyDefinition) {
    ...
    private Collection<? extends AggregationBuilder> getElasticsearchCalendarFieldAggregation(CalendarFieldAggregation agg) {
        List<AggregationBuilder> aggs = new ArrayList<>();
        org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(agg.getPropertyName());
        if (propertyDefinition == null) {
