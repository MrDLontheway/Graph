...
public class DefaultIndexSelectionStrategy implements IndexSelectionStrategy {
    ...
    public static final String CONFIG_SPLIT_Date = "splitDate";
    ...
    private final boolean splitIndexByDate;
    ...
    public DefaultIndexSelectionStrategy(GraphConfiguration config) {
        splitIndexByDate = getSplitDate(config);
    ...
    private static boolean getSplitDate(GraphConfiguration config) {
        boolean splitEdgesAndVertices = config.getBoolean(GraphConfiguration.SEARCH_INDEX_PROP_PREFIX + "." + CONFIG_SPLIT_Date, DEFAULT_SPLIT_VERTICES_AND_EDGES);
        LOGGER.info("Split edges and vertices: %s", splitEdgesAndVertices);
        return splitEdgesAndVertices;
    }
...
//                if (indexName.startsWith(defaultIndexName)){
//                    newIndicesToQuery.add(indexName);
//                }
            }
    ...
    public String getIndexName(Elasticsearch5SearchIndex es, Element element) {
        ...
        if (splitEdgesAndVertices) {
            if (element instanceof Vertex) {
                resIndex = defaultIndexName + VERTICES_INDEX_SUFFIX_NAME;
            } else if (element instanceof Edge) {
                resIndex = defaultIndexName + EDGES_INDEX_SUFFIX_NAME;
            } else {
                throw new VertexiumException("Unhandled element type: " + element.getClass().getName());
            }
        }else {
            resIndex = defaultIndexName;
        }

        if (splitIndexByDate) {
            String format = simpleDateFormat.format(new Date(element.getTimestamp()));
            resIndex += "-"+format;
        }
        return resIndex;
    }
