...
public abstract class QueryParameters {
    ...
    public void addHasContainer(QueryBase.HasContainer hasContainer) {
        ...
        if(hasContainer instanceof QueryBase.HasPropertyContainer){
            ((QueryBase.HasPropertyContainer) hasContainer).getKeys().forEach(x -> {
                mykeys.add(x.replace(".","-"));
            });
            hasContainer = new QueryBase.HasPropertyContainer(mykeys);
        }else if (hasContainer instanceof QueryBase.HasValueContainer){
            ((QueryBase.HasValueContainer) hasContainer).getKeys().forEach(x -> {
                mykeys.add(x.replace(".","-"));
            });
            hasContainer = new QueryBase.HasValueContainer(mykeys,((QueryBase.HasValueContainer) hasContainer).predicate,((QueryBase.HasValueContainer) hasContainer).value,((QueryBase.HasValueContainer) hasContainer).propertyDefinitions);
        }else if(hasContainer instanceof QueryBase.HasNotPropertyContainer){
            ((QueryBase.HasNotPropertyContainer) hasContainer).getKeys().forEach(x -> {
                mykeys.add(x.replace(".","-"));
            });
            hasContainer = new QueryBase.HasNotPropertyContainer(mykeys);
        }
