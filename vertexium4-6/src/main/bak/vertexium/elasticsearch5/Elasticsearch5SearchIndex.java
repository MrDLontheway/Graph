...
public class Elasticsearch5SearchIndex implements SearchIndex, SearchIndexWithVertexPropertyCountByValue {
    ...
    //TODO __location
//    public static final String ELEMENT_LOCATION = "__location";
    //
    ...
    private <TElement extends Element> IndexInfo addMutationPropertiesToIndex(Graph graph, ExistingElementMutation<TElement> mutation) {
        ...
        mutation.getAlterPropertyVisibilities().stream()
                .filter(p -> p.getExistingVisibility() != null && !p.getExistingVisibility().equals(p.getVisibility()))
                .forEach(p -> {
                    org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(graph, p.getName());
    ...
    private XContentBuilder buildJsonContentFromElement(Graph graph, Element element, Authorizations authorizations) throws IOException {
        ...
        if (element instanceof Vertex) {
            //TODO DL CHANGGE buildJsonContentFromElement
//            Object location = element.getPropertyValue("location");
//            if(location!=null){
//                String[] split = location.toString().split(",");
//                jsonBuilder.field(ELEMENT_LOCATION,new org.elasticsearch.common.geo.GeoPoint(Double.valueOf(split[0]),Double.valueOf(split[1])));
//            }
            //DL CHANGGE
...
//            }
    ...
    private void addValuesToFieldMap(Graph graph, Map<String, Object> propertiesMap, String propertyName, Object propertyValue) {
        org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(graph, propertyName);
    ...
    public String replaceFieldnameDots(String fieldName) {
        return fieldName;//TODO DL REMOVE .replace(".", FIELDNAME_DOT_REPLACEMENT);
    }
    ...
    //todo to public
    public static List<String> getQueryableTypeSuffixes(org.vertexium.PropertyDefinition propertyDefinition) {
    ...
    public GraphQuery queryGraph(Graph graph, String queryString, Authorizations authorizations) {
        return new ElasticsearchSearchGraphQuery(
                ...
                graph,
                queryString,
                new org.vertexium.elasticsearch5.ElasticsearchSearchQueryBase.Options()
    ...
    protected void addPropertyDefinitionToIndex(Graph graph, IndexInfo indexInfo, String propertyName, Visibility propertyVisibility, org.vertexium.PropertyDefinition propertyDefinition) throws IOException {
    ...
    protected org.vertexium.PropertyDefinition getPropertyDefinition(Graph graph, String propertyName) {
    ...
    public Map<Object, Long> getVertexPropertyCountByValue(Graph graph, String propertyName, Authorizations authorizations) {
        ...
        for (String p : getAllMatchingPropertyNames(graph, propertyName, authorizations)) {
            ...
            org.vertexium.PropertyDefinition propertyDefinition = getPropertyDefinition(graph, p);
        ...
        if (org.vertexium.elasticsearch5.ElasticsearchSearchQueryBase.QUERY_LOGGER.isTraceEnabled()) {
            org.vertexium.elasticsearch5.ElasticsearchSearchQueryBase.QUERY_LOGGER.trace("query: %s", q);
        }
    ...
    protected void createIndexAddFieldsToElementType(XContentBuilder builder) throws IOException {
        ...
        //TODO ES create Index Mapping add GeoPoint
//                .startObject(ELEMENT_LOCATION).field("type", "geo_point").endObject()
        //ES create Index Mapping add GeoPoint
        ;
    }

    @Override
    public void deleteProperty(Graph graph, Element element, org.vertexium.PropertyDescriptor property, Authorizations authorizations) {
        deleteProperties(graph, element, Collections.singletonList(property), authorizations);
    }

    @Override
    public void deleteProperties(Graph graph, Element element, Collection<org.vertexium.PropertyDescriptor> propertyList, Authorizations authorizations) {
        List<String> fieldsToRemove = new ArrayList<>();
        Map<String, Object> fieldsToSet = new HashMap<>();
